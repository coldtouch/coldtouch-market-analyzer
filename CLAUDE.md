...
- [ ] **`recipes.json` weapon secondary-material audit (verify in-game first)** — discovered 2026-06-22 while building a crafting mats order. The app's `recipes.json` lists **maces/hammers as `METALBAR + LEATHER`**, but the authoritative `../ao-bin-dumps/items.json` (raw game data) says **`METALBAR + CLOTH`** (e.g. `T6_2H_MACE` → 20 metalbar + 12 cloth; `T6_2H_HAMMER` same; `T6_MAIN_SWORD` correctly stays metalbar+leather). So Crafting Profits is using the wrong secondary mat for the mace/hammer line. **User wants to confirm in-game before changing** (open the crafting station, check the actual mats) so the fix is 100% accurate. Action when confirmed: (a) audit ALL weapon classes' secondary mat in `recipes.json` against ao-bin-dumps (the bug may not be limited to mace/hammer), (b) correct the mismatches, (c) re-verify Crafting Profits costs. Cross-check script pattern: load `ao-bin-dumps/items.json` → `items.weapon[*].craftingrequirements.craftresource`.
