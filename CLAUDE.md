...
### Pending
- [ ] **SQLITE_BUSY root cause fix — STILL OPEN (recurred April 23).** `busy_timeout=30000` on all three connections (db/statsDb/readDb) was NOT sufficient. Site went down again April 23 ~19:00 UTC with the same cascade: `[FATAL] Uncaught exception: SQLITE_BUSY` repeating every 30s, then 13-minute silent stretches (event loop wedged), ports 80/443 bound but no HTTP responses. Resolved again by `systemctl restart albion-saas` (old PID consumed 11.1 GB peak memory — near heap limit). Real fix needed: (a) the `uncaughtException` handler in [deploy_saas.py](deploy_saas.py) swallows `SQLITE_BUSY` and returns to event loop, but the half-broken transaction context leaves subsequent queries in a wedged state — need to either terminate the process on SQLITE_BUSY (let systemd restart us cleanly) or wrap each DB call with a retry-then-bubble-up pattern that doesn't leave dangling promises; (b) investigate why memory climbs to 11 GB — likely an unresolved promise chain from the WAL contention; (c) cross-guard audit: `statsRunning`/`analyticsRunning` flags may be falsely clearing while a write batch is still in-flight.
